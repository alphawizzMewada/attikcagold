  ...
  addAddress() async {
    try {
      ...
      Map params = {
       ...
       // "product_id": widget.model.itemList[0].id.toString(),
        "product_id": productId.toString().replaceAll("[", "").replaceAll("]", ""),
        "seller_id": widget.model.orderItems![0].sellerId.toString(),
        "comment": controller.text.toString(),
        "user_retting": value.toString(),
      };
      Map response =
      await apiBase.postAPICall(Uri.parse(baseUrl + "order_details_review"), params);
      setState(() {
        loading = true;
      });
